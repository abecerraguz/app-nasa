    ...
    await axios.get('https://app-shopbikes.herokuapp.com/ordenes')
        .then( result => {
            ...
            info.forEach(element =>{
                ...
                <td>
                <button type="button" class="btn btn-danger btn-sm" id="${element.product_id}" onclick='infoProducto("Mierda")'>
...
// Activación de una Modal
// UI.modalButton.forEach(element =>{
...
axios.get('https://app-shopbikes.herokuapp.com/categorias')
    .then( result =>{
        ...
        categorias.forEach( element => {
            UI.category_id.innerHTML+= `<option value="${element.category_id}">${element.category_name}</option>`
        });
})
